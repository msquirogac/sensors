...
{
	MS5611_GetCalibParams(hsensor);
}

void MS5611_Reset(MS5611_HandleTypeDef *hsensor)
{
	MS5611_CommandHandler cmd = hsensor->HAL.Command;
...
{
	MS5611_CommandHandler cmd = hsensor->HAL.Command;
	MS5611_CParamsTypeDef *cparams = &hsensor->CParams;
	uint8_t tmpreg[2];
...
{
	MS5611_CommandHandler cmd = hsensor->HAL.Command;
	MS5611_InitTypeDef *init = &hsensor->Init;
...
{
	MS5611_CParamsTypeDef *cparams = &hsensor->CParams;
	MS5611_IntVarsTypeDef *intvars = &hsensor->IntVars;
	int32_t temperature;
