...
{
	MS5611_SendCommand(MS5611_CMD_RESET, NULL, 0);
}
...
void MS5611_GetCalibParams(MS5611_HandleTypeDef *ms5611)
{
	...
	uint8_t tmpreg[2];
	MS5611_SendCommand(MS5611_CMD_PROM_READ | 0x02, tmpreg, 2);
	cparams->C1_SENS_T1 = (tmpreg[0] << 8) | (tmpreg[1] << 0);
	MS5611_SendCommand(MS5611_CMD_PROM_READ | 0x04, tmpreg, 2);
	cparams->C2_OFF_T1 = (tmpreg[0] << 8) | (tmpreg[1] << 0);
	MS5611_SendCommand(MS5611_CMD_PROM_READ | 0x06, tmpreg, 2);
	cparams->C3_TCS = (tmpreg[0] << 8) | (tmpreg[1] << 0);
	MS5611_SendCommand(MS5611_CMD_PROM_READ | 0x07, tmpreg, 2);
	cparams->C4_TCO = (tmpreg[0] << 8) | (tmpreg[1] << 0);
	MS5611_SendCommand(MS5611_CMD_PROM_READ | 0x0A, tmpreg, 2);
	cparams->C5_TREF = (tmpreg[0] << 8) | (tmpreg[1] << 0);
	MS5611_SendCommand(MS5611_CMD_PROM_READ | 0x0C, tmpreg, 2);
...
void MS5611_RequestTemperature(MS5611_HandleTypeDef *ms5611)
{
	...
	MS5611_SendCommand(MS5611_CMD_ADC_CONV | MS5611_CMD_ADC_D2 | init->OSRSelectTemp, NULL, 0);
}

void MS5611_RequestPressure(MS5611_HandleTypeDef *ms5611)
{
	MS5611_InitTypeDef *init = &ms5611->Init;
	MS5611_SendCommand(MS5611_CMD_ADC_CONV | MS5611_CMD_ADC_D1 | init->OSRSelectPress, NULL, 0);
}
...
uint32_t MS5611_GetADC(MS5611_HandleTypeDef *ms5611)
{
	uint32_t adcvalue;
	uint8_t tmpreg[3];
	MS5611_SendCommand(MS5611_CMD_ADC_READ, tmpreg, 3);
